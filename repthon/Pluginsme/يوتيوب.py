...
async def venom_download(url: str, mode: str) -> tuple[Path | None, Path | None, dict | None]:
    ...
    - الصوت: يُحمّل كـ bestaudio ثم يُحوَّل إلى MP3 (الصيغة المفضلة لتيليجرام).
    - الفيديو: يُحمّل ويُدمَج إلى MP4.
    - العميل: web فقط.
    """
    is_ultra = bool(gvarstatus("VENOM_ULTRA_HQ"))
    
    if mode == "audio":
        # نجلب أفضل صوت متاح ثم نحوله إلى MP3
        fmt = "bestaudio/best"
        ext = "mp3"
        postprocessors = [{
        ...
        # فيديو: نجلب أفضل فيديو + صوت وندمجهم إلى MP4
        fmt = (
            "bv*[ext=mp4]+ba[ext=m4a]/b[ext=mp4]/best"
            if is_ultra
            else "bv*[height<=720][ext=mp4]+ba[ext=m4a]/b[ext=mp4]/best"
        )
        ext = "mp4"
        postprocessors = []
        merge_output = "mp4"
    ...
    def _dl():
        with yt_dlp.YoutubeDL(opts) as ydl:
            ...
            thumb_path = next(
                (base_name.with_suffix(img_ext) for img_ext in [".jpg", ".webp", ".png"]
                 if base_name.with_suffix(img_ext).exists()),
                None
            )

            # بعد المعالجة اللاحقة (التحويل إلى mp3)، قد يتغير الامتداد
...
async def yt_search_audio(event):
    ...
    if not query:
        return await edit_delete(event, "**⤶ يرجى كتابة اسم الأغنية**")
    
    zed = await edit_or_reply(event, "**⪼ جاري البحث وتحميل الصوت (mp3) ... 🎧**")
...
async def yt_inline_dl(event):
    ...
    if not query:
        return await edit_delete(event, "**⤶ يرجى إدراج المقطع أو الرابط**")
    ...
    buttons = [[
        Button.inline("صوت 🎧", data=b"ytdl_audio"),
        Button.inline("فيديو 🎬", data=b"ytdl_video")
    ]]
...
async def yt_search_video(event):
    ...
    query = event.pattern_match.group(1).strip()
    if not query:
        return await edit_delete(event, "**⤶ يرجى كتابة اسم المقطع**")
...
async def yt_link_audio(event):
    ...
    if not url:
        return await edit_delete(event, "**⤶ يرجى وضع الرابط**")
...
async def all_social_dl(event):
    ...
    if not url:
        return await edit_delete(event, f"**⤶ يرجى وضع رابط {platform}**")
    ...
    if path and path.exists():
        ...
        await zed.edit(f"**⤶ فشل التحميل من {platform}.**")
...
async def search_pic(event):
    query = event.pattern_match.group(1).strip()
    if not query:
        return await edit_delete(event, "**⤶ اكتب ما تريد البحث عنه**")
...
async def on_ytdl_cb(event):
    ...
    await zq_lo.send_file(
        ...
        thumb=str(thumb) if thumb else None,
        attributes=attr,
        supports_streaming=(action == "video")
    )
